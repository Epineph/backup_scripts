...
backup_file() {
    ...
    local backup_dir="$HOME/.backup_scripts/$date_dir"
    local file_name=$(basename "$file_path")
    local backup_name="$backup_dir/${file_name}_backup"

    mkdir -p "$backup_dir"

    local count=1
    while [[ -f "${backup_name}_$count" ]]; do
        ((count++))
    done
    backup_name="${backup_name}_$count"
...
# Index backups after script execution
index_backups
